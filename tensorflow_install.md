# <center>Tensorflow install turial</center>

## 1. import erro 

    >>> import tensorflow
    2026-01-21 10:41:49.725811: W tensorflow/stream_executor/platform/default/dso_loader.cc:59] Could not load dynamic library 'libcudart.so.10.1'; dlerror: libcudart.so.10.1: cannot open shared object file: No such file or directory; LD_LIBRARY_PATH: /home/user/program/lib:/home/user/program/lib64:/home/user/software/gromacs-2022/lib64:/home/user/miniconda3/envs/afd/lib/python3.8/site-packages/tensorrt_libs
    2026-01-21 10:41:49.725839: I tensorflow/stream_executor/cuda/cudart_stub.cc:29] Ignore above cudart dlerror if you do not have a GPU set up on your machine.

### enviroment:

    name: TAWFN_test
    channels:
        - rdkit
        - pytorch
        - omgarcia
        - http://mirrors.tuna.tsinghua.edu.cn/anaconda/cloud/conda-forge
        - salilab
        - anaconda
        - defaults
    dependencies:
        - _libgcc_mutex=0.1=main
        - backcall=0.2.0=pyhd3eb1b0_0
        - blas=1.0=mkl
        - bzip2=1.0.8=h7b6447c_0
        - ca-certificates=2024.7.2=h06a4308_0
        - cairo=1.16.0=hf32fb01_1
        - certifi=2022.12.7=py37h06a4308_0
        - cudatoolkit=10.2.89=hfd86e86_1
        - cudnn=7.6.5=cuda10.2_0
        - cycler=0.10.0=py37_0
        - dbus=1.13.18=hb2f20db_0
        - decorator=5.0.9=pyhd3eb1b0_0
        - dssp=3.0.0=hf484d3e_3
        - easydict=1.9=py_0
        - expat=2.4.1=h2531618_2
        - fontconfig=2.13.1=h6c09931_0
        - freetype=2.10.4=h5ab3b9f_0
        - gcc-6=6.1.0=2
        - glib=2.68.2=h36276a3_0
        - gmp=6.1.0=1
        - gst-plugins-base=1.14.0=h8213a91_2
        - gstreamer=1.14.0=h28cd5cc_2
        - icu=58.2=he6710b0_3
        - intel-openmp=2021.2.0=h06a4308_610
        - ipython=7.22.0=py37hb070fc8_0
        - ipython_genutils=0.2.0=pyhd3eb1b0_1
        - isl=0.17.1=0
        - jedi=0.17.0=py37_0
        - joblib=1.0.1=pyhd3eb1b0_0
        - jpeg=9b=h024ee3a_2
        - kiwisolver=1.3.1=py37h2531618_0
        - lcms2=2.12=h3be6417_0
        - ld_impl_linux-64=2.33.1=h53a641e_7
        - libboost=1.67.0=h46d08c1_4
        - libffi=3.3=he6710b0_2
        - libgcc=7.2.0=h69d50b8_2
        - libgcc-ng=9.1.0=hdf63c60_0
        - libgfortran-ng=7.3.0=hdf63c60_0
        - libpng=1.6.37=hbc83047_0
        - libstdcxx-ng=9.1.0=hdf63c60_0
        - libtiff=4.2.0=h85742a9_0
        - libuuid=1.0.3=h1bed415_2
        - libuv=1.40.0=h7b6447c_0
        - libwebp-base=1.2.0=h27cfd23_0
        - libxcb=1.14=h7b6447c_0
        - libxml2=2.9.10=hb55368b_3
        - lz4-c=1.9.3=h2531618_0
        - matplotlib=3.3.4=py37h06a4308_0
        - matplotlib-base=3.3.4=py37h62a2d02_0
        - mkl=2021.2.0=h06a4308_296
        - mkl-service=2.3.0=py37h27cfd23_1
        - mkl_fft=1.3.0=py37h42c9631_2
        - mkl_random=1.2.1=py37ha9443f7_2
        - mpc=1.0.1=0
        - mpfr=3.1.2=0
        - ncurses=6.2=he6710b0_1
        - networkx=2.5=py_0
        - ninja=1.10.2=hff7bd54_1
        - olefile=0.46=py37_0
        - openssl=1.1.1w=h7f8727e_0
        - pandas=1.2.4=py37h2531618_0
        - parso=0.8.2=pyhd3eb1b0_0
        - pcre=8.44=he6710b0_0
        - pexpect=4.8.0=pyhd3eb1b0_3
        - pickleshare=0.7.5=pyhd3eb1b0_1003
        - pillow=8.2.0=py37he98fc37_0
        - pip=21.1.1=py37h06a4308_0
        - pixman=0.40.0=h7b6447c_0
        - prompt-toolkit=3.0.17=pyh06a4308_0
        - ptyprocess=0.7.0=pyhd3eb1b0_2
        - py-boost=1.67.0=py37h04863e7_4
        - pygments=2.9.0=pyhd3eb1b0_0
        - pyparsing=2.4.7=pyhd3eb1b0_0
        - pyqt=5.9.2=py37h05f1152_2
        - python=3.7.10=hdb3f193_0
        - python-dateutil=2.8.1=pyhd3eb1b0_0
        - python_abi=3.7=1_cp37m
        - pytorch=1.7.0=py3.7_cuda10.2.89_cudnn7.6.5_0
        - pytz=2021.1=pyhd3eb1b0_0
        - qt=5.9.7=h5867ecd_1
        - rdkit=2020.03.2.0=py37hc20afe1_1
        - readline=8.1=h27cfd23_0
        - scikit-learn=0.24.2=py37ha9443f7_0
        - scipy=1.6.2=py37had2a1c9_1
        - sip=4.19.8=py37hf484d3e_0
        - six=1.15.0=py37h06a4308_0
        - sqlite=3.35.4=hdfb4753_0
        - threadpoolctl=2.1.0=pyh5ca1d4c_0
        - tk=8.6.10=hbc83047_0
        - tornado=6.1=py37h27cfd23_0
        - tqdm=4.59.0=pyhd3eb1b0_1
        - traitlets=5.0.5=pyhd3eb1b0_0
        - typing_extensions=3.7.4.3=pyha847dfd_0
        - wcwidth=0.2.5=py_0
        - wheel=0.36.2=pyhd3eb1b0_0
        - xz=5.2.5=h7b6447c_0
        - zlib=1.2.11=h7b6447c_3
        - zstd=1.4.9=haebb681_0
        - pip:
            - absl-py==2.1.0
            - ase==3.21.1
            - astunparse==1.6.3
            - cached-property==1.5.2
            - cachetools==5.3.0
            - chardet==4.0.0
            - click==7.1.2
            - dataclasses==0.6
            - deepgoplus==1.0.1
            - fair-esm==2.0.0
            - future==0.18.2
            - gast==0.3.3
            - google-auth==2.16.3
            - google-auth-oauthlib==0.4.6
            - google-pasta==0.2.0
            - googledrivedownloader==0.4
            - grpcio==1.53.0
            - h5py==2.10.0
            - idna==2.10
            - importlib-metadata==6.1.0
            - isodate==0.6.0
            - jinja2==3.0.1
            - keras-preprocessing==1.1.2
            - llvmlite==0.36.0
            - markdown==3.4.3
            - markupsafe==2.1.2
            - numba==0.53.1
            - numpy==1.18.5
            - oauthlib==3.2.2
            - opt-einsum==3.3.0
            - protobuf==3.20.3
            - psutil==5.9.4
            - pyasn1==0.4.8
            - pyasn1-modules==0.2.8
            - python-louvain==0.13
            - pyyaml==5.3.1
            - rdflib==5.0.0
            - requests==2.25.1
            - requests-oauthlib==1.3.1
            - rsa==4.9
            - setuptools==52.0.0
            - sklearn==0.0
            - tensorboard==2.11.2
            - tensorboard-data-server==0.6.1
            - tensorboard-plugin-wit==1.8.1
            - tensorflow==2.3.4
            - tensorflow-estimator==2.3.0
            - termcolor==2.2.0
            - torch-cluster==1.5.9
            - torch-geometric==1.6.3
            - torch-scatter==2.0.7
            - torch-sparse==0.6.9
            - torch-spline-conv==1.2.1
            - urllib3==1.26.5
            - werkzeug==2.2.3
            - wrapt==1.15.0
            - zipp==3.15.0
  
### tensorflow-2.3.4 对应cudatoolkit-10.1， 而环境中安装的是cudatoolkit-10.2，对应的动态库为：

    libcudadevrt.a
    libcudart.so.10.2.89
    libcudart.so.10.2 -> libcudart.so.10.2.89
    libcudart.so -> libcudart.so.10.2.89

### 解决动态库调用问题只需要创建一个软连接：

    ln -s libcudart.so.10.2.89 libcudart.so.10.1

### 动态库如下：

    libcudadevrt.a
    libcudart.so.10.2.89
    libcudart.so.10.2 -> libcudart.so.10.2.89
    libcudart.so -> libcudart.so.10.2.89
    libcudart.so.10.1 -> libcudart.so.10.2.89

### 重新导入tensorflow，问题解决。

    $ python
    Python 3.7.10 (default, Feb 26 2021, 18:47:35) 
    [GCC 7.3.0] :: Anaconda, Inc. on linux
    Type "help", "copyright", "credits" or "license" for more information.
    >>> import tensorflow
    2026-01-21 10:56:57.323952: I tensorflow/stream_executor/platform/default/dso_loader.cc:48] Successfully opened dynamic library libcudart.so.10.1
    >>> import tensorflow
    >>>

# *****安装更高版本的tensorflow-2.10.0 *****

### 此处与网络搜到的版本对应关系（<https://blog.csdn.net/ly869915532/article/details/124542362>）有出入，以此为准：

    conda create -n tensorflow python=3.9 -y

    conda activate tensorflow

    conda install cudatoolkit=11.3 cudnn=8.2 -y
    
    pip install pybind11

    conda install tensorflow=2.10.0=gpu_py39h039f4ff_0 -y

    
    /* pip install biopython scikit-learn */

### 注意：使用pip安装tensorflow 如果提示动态库找不到，是因为编译时用的Python环境是通过miniconda构建的虚拟环境，所以 Python 3.9 可能安装在了一个非标准的路径下，而这个路径没有被包含在动态链接器的搜索路径中，使用conda安装可以解决问题。

